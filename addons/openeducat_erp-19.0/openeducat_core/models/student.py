...
class OpStudentCourse(models.Model):
    ...
    _unique_name_roll_number_student_id = models.Constraint(
        "unique(student_id,course_id,batch_id)", "Student must be unique per Batch!"
    )
...
class OpStudent(models.Model):
    ...
    _unique_gr_no = models.Constraint(
        "unique(gr_no)", "Registration Number must be unique per student!"
    )
        ...
        for record in self:
            if record.email:
                duplicate = self.env["res.partner"].search(
                    [("email", "=", record.email), ("id", "!=", record.partner_id.id)],
                    limit=1,
                )
                if duplicate:
