...
class OpAcademicTerm(models.Model):
    ...
    _unique_name = models.Constraint(
        "UNIQUE(name, academic_year_id)", "Name must be unique per Academic Year."
    )
    _unique_start_date = models.Constraint(
        "UNIQUE(term_start_date, academic_year_id)",
        "Start date must be unique per Academic Year.",
    )
    _unique_end_date = models.Constraint(
        "UNIQUE(term_end_date, academic_year_id)",
