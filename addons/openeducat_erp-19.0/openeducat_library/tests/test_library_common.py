...
class TestLibraryCommon(common.TransactionCase):
    def setUp(self):
        ...
        self.author = self.op_author.search(
            [("name", "=", "Test Author")], limit=1
        ) or self.op_author.create({"name": "Test Author"})
        self.publisher = self.op_publisher.search(
            [("name", "=", "Test Publisher")], limit=1
        ) or self.op_publisher.create({"name": "Test Publisher"})
        self.media_type = self.op_media_type.search(
            [("name", "=", "Book")], limit=1
        ) or self.op_media_type.create({"name": "Book", "code": "B1"})
        self.media = self.op_media.search(
            [("name", "=", "Test Book")], limit=1
        ) or self.op_media.create(
            {
                "name": "Test Book",
        ...
        self.course = self.env["op.course"].create(
            {"name": "Library Course", "code": "LC1"}
        )
        self.subject = self.env["op.subject"].create(
            {"name": "Library Subject", "code": "LS1"}
        )
