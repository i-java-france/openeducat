...
class TestParentCommon(common.TransactionCase):
    def setUp(self):
        ...
        self.rel_father = self.env["op.parent.relationship"].search(
            [("name", "=", "Test Father")], limit=1
        )
        if not self.rel_father:
            self.rel_father = self.env["op.parent.relationship"].create(
                {"name": "Test Father"}
            )

        self.rel_mother = self.env["op.parent.relationship"].search(
            [("name", "=", "Test Mother")], limit=1
        )
        if not self.rel_mother:
            self.rel_mother = self.env["op.parent.relationship"].create(
                {"name": "Test Mother"}
            )
