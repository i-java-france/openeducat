...
class TestAsset(TestClassroomCommon):
    ...
    def test_case_1_asset(self):
        ...
        classroom = self.env.ref(
            "openeducat_classroom.op_classroom_1", raise_if_not_found=False
        ) or self.env["op.classroom"].search([], limit=1)
        ...
        if not classroom:
            classroom = self.env["op.classroom"].create(
                {"name": "Test Classroom", "code": "TCR-01", "capacity": 30}
            )
