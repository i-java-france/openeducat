...
class TestTimetableRobustCommon(TransactionCase):
    def setUp(self):
        ...
        self.classroom = self.Classroom.create(
            {"name": "Room 101", "code": "R101", "capacity": 30}
        )
...
class TestTimingModel(TestTimetableRobustCommon):
    ...
    def test_01_timing_creation(self):
        ...
        t = self.Timing.create(
            {"name": "Evening", "hour": "5", "minute": "30", "am_pm": "pm"}
        )
    ...
    def test_02_timing_sequence(self):
        ...
        t = self.Timing.create(
            {"name": "Seq Test", "hour": "10", "minute": "00", "am_pm": "am"}
        )
...
class TestTimetableConflicts(TestTimetableRobustCommon):
    ...
    def setUp(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param(
            "timetable.is_faculty_constraint", "True"
        )
        self.env["ir.config_parameter"].sudo().set_param(
            "timetable.is_classroom_constraint", "True"
        )
        self.env["ir.config_parameter"].sudo().set_param(
            "timetable.is_batch_constraint", "True"
        )
    ...
    def test_02_classroom_conflict(self):
        ...
        dt = fields.Datetime.now().replace(
