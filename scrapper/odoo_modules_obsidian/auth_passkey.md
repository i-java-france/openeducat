...
> **Module:** `auth_passkey` | **Version:** `19.0.1.1` **Category:** Tools |
...
Passkeys are a secure alternative to a username and a password. When a user logs in with
a Passkey, MFA will not be required.
...
- `* INHERIT Passkey Login OAuth (qweb)`
- `* INHERIT auth.passkey.users.form (form)`
- `* INHERIT auth.passkey.users.preferences (form)`
- `* INHERIT auth_passkey_login (qweb)`
- `* INHERIT res.users.identitycheck form (form)`
- `Create Passkey Wizard Form (form)`
- `auth.passkey.key.kanban (kanban)`
- `auth.passkey.key.rename (form)`
...
### 🗃️ `auth.passkey.key.create` — Create a Passkey
...
| Name                                  | Group         | Perms     |
| ------------------------------------- | ------------- | --------- |
| access_auth_passkey_key_create_portal | Role / Portal | `R/W/C/D` |
| access_auth_passkey_key_create        | Role / User   | `R/W/C/D` |

**Record Rules:**

- **Passkeys: Users can only modify their own Passkey creation requests** (10, 1)
  `R/W/C/D`
  - Domain: `[('create_uid', '=', user.id)]`

### 🗃️ `auth.passkey.key` — Passkey
...
| `credential_identifier` | Credential Identifier | `char`     | ✅  | ✅    |           |
| `display_name`          | Display Name          | `char`     |     |       |           |
| `id`                    | ID                    | `integer`  |     | ✅    |           |
| `name`                  | Name                  | `char`     | ✅  | ✅    |           |
| `public_key`            | Public Key            | `char`     | ✅  |       |           |
| `sign_count`            | Sign Count            | `integer`  |     | ✅    |           |
| `write_date`            | Last Updated on       | `datetime` |     | ✅    |           |
| `write_uid`             | Last Updated by       | `many2one` |     | ✅    | res.users |
...
| Name                           | Group         | Perms   |
| ------------------------------ | ------------- | ------- |
| access_auth_passkey_key_admin  | Access Rights | `R/W/D` |
| access_auth_passkey_key_portal | Role / Portal | `R/W`   |
| access_auth_passkey_key        | Role / User   | `R/W`   |

**Record Rules:**

- **Passkeys: Users can only access own Passkeys** (10, 1) `R/W/C/D`
  - Domain: `[('create_uid', '=', user.id)]`
- **Passkeys: Admins can view and delete other peoples Passkeys** (2) `R/D`
  - Domain: `[(1, '=', 1)]`

### 🗃️ `res.users.identitycheck` — Password Check Wizard

> ✳️ Transient (Wizard)

Wizard used to re-check the user's credentials (password) and eventually revoke access
to his account to every device he has an active session on.
...
| Field          | Label           | Type        | Req | Store | Relation  |
| -------------- | --------------- | ----------- | --- | ----- | --------- |
| `auth_method`  | Auth Method     | `selection` |     | ✅    |           |
| `create_date`  | Created on      | `datetime`  |     | ✅    |           |
| `create_uid`   | Created by      | `many2one`  |     | ✅    | res.users |
| `display_name` | Display Name    | `char`      |     |       |           |
| `id`           | ID              | `integer`   |     | ✅    |           |
...
| Name               | Group         | Perms   |
| ------------------ | ------------- | ------- |
| id check portal    | Role / Portal | `R/W/C` |
| id check employees | Role / User   | `R/W/C` |

**Record Rules:**

- **users can only access their own id check** (Global) `R/W/C/D`
  - Domain: `[('create_uid', '=', user.id)]`
