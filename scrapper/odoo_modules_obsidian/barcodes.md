...
> **Module:** `barcodes` | **Version:** `19.0.2.0` **Category:** Inventory |
...
### 🗃️ `barcodes.barcode_events_mixin` — Barcode Event Mixin

Mixin class for objects reacting when a barcode is scanned in their form views which
contains `<field name="_barcode_scanned" widget="barcode_handler"/>`. Models using this
mixin must implement the method on_barcode_scanned. It works like an onchange and
...
| Field              | Label           | Type      | Req | Store | Relation |
| ------------------ | --------------- | --------- | --- | ----- | -------- |
| `_barcode_scanned` | Barcode Scanned | `char`    |     |       |          |
| `display_name`     | Display Name    | `char`    |     |       |          |
| `id`               | ID              | `integer` |     | ✅    |          |
