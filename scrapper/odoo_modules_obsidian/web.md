...
> **Module:** `web` | **Version:** `19.0.1.0` **Category:** Technical | **License:**
...
- `* INHERIT Frontend Layout (qweb)`
- `* INHERIT Neutralize Banner (qweb)`
- `* INHERIT ir.ui.view.form.inherit (form)`
...
- `web.unit_tests_suite (qweb)`
- `web.webclient_bootstrap (qweb)`
- `webclient_offline (qweb)`
- `webclient_scoped_app (qweb)`
...
### 🗃️ `properties.base.definition` — Properties Base Definition
...
| `display_name`          | Display Name          | `char`                  |     |       |                 |
| `id`                    | ID                    | `integer`               |     | ✅    |                 |
| `properties_definition` | Properties Definition | `properties_definition` |     | ✅    |                 |
| `properties_field_id`   | Properties Field      | `many2one`              | ✅  | ✅    | ir.model.fields |
| `write_date`            | Last Updated on       | `datetime`              |     | ✅    |                 |
| `write_uid`             | Last Updated by       | `many2one`              |     | ✅    | res.users       |
...
| Name                              | Group                  | Perms     |
| --------------------------------- | ---------------------- | --------- |
| access_properties_base_definition | Email Marketing / User | `R/W/C/D` |
| access_properties_base_definition | Role / Administrator   | `R/W/C/D` |

**Record Rules:**

- **properties.base.definition: system all access** (4) `R/W/C/D`
  - Domain: `[(1, '=', 1)]`
- **properties.base.definition: mailing user** (53) `R/W/C/D`
  - Domain:
    `[('properties_field_id', '=', user.env.ref('mass_mailing.field_mailing_contact__properties').id)]`

### 🗃️ `ir.qweb.field.image_url` — Qweb Field Image

`image_url` widget rendering, inserts an image tag in the document.
...
### 🗃️ `res.users.settings.embedded.action` — User Settings for Embedded Actions
...
| `create_uid`                  | Created by                              | `many2one` |     | ✅    | res.users             |
| `display_name`                | Display Name                            | `char`     |     |       |                       |
| `embedded_actions_order`      | List order of embedded action ids       | `char`     |     | ✅    |                       |
| `embedded_actions_visibility` | List visibility of embedded actions ids | `char`     |     | ✅    |                       |
| `embedded_visibility`         | Is top bar visible                      | `boolean`  |     | ✅    |                       |
| `id`                          | ID                                      | `integer`  |     | ✅    |                       |
| `res_id`                      | Res                                     | `integer`  |     | ✅    |                       |
| `res_model`                   | Res Model                               | `char`     | ✅  | ✅    |                       |
...
| Name                               | Group       | Perms     |
| ---------------------------------- | ----------- | --------- |
| res.users.settings.embedded.action | Role / User | `R/W/C/D` |

**Record Rules:**

- **res.users.settings.embedded.action: access their own entries** (1) `R/W/C/D`
  - Domain: `[('user_setting_id.user_id', '=', user.id)]`
- **Administrators can access all User Settings embedded actions** (4) `R/W/C/D`
  - Domain: `[(1, '=', 1)]`
