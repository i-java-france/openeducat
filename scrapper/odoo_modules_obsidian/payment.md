...
> **Module:** `payment` | **Version:** `19.0.2.0` **Category:** Technical | **License:**
...
- `Submit Button (qweb)`
- `availability_report_records (qweb)`
- `payment.availability_report (qweb)`
- `payment.availability_report_button (qweb)`
...
| Field                               | Label                             | Type        | Req | Store | Relation          |
| ----------------------------------- | --------------------------------- | ----------- | --- | ----- | ----------------- |
| `address_format`                    | Layout in Reports                 | `text`      |     | ✅    |                   |
| `address_view_id`                   | Input View                        | `many2one`  |     | ✅    | ir.ui.view        |
| `code`                              | Country Code                      | `char`      | ✅  | ✅    |                   |
| `country_group_codes`               | Country Group Codes               | `json`      |     |       |                   |
...
| `id`                                | ID                                | `integer`   |     | ✅    |                   |
| `image_url`                         | Flag                              | `char`      |     |       |                   |
| `is_mercado_pago_supported_country` | Is Mercado Pago Supported Country | `boolean`   |     |       |                   |
| `is_stripe_supported_country`       | Is Stripe Supported Country       | `boolean`   |     |       |                   |
| `name`                              | Country Name                      | `char`      | ✅  | ✅    |                   |
| `name_position`                     | Customer Name Position            | `selection` |     | ✅    |                   |
| `phone_code`                        | Country Calling Code              | `integer`   |     | ✅    |                   |
| `state_ids`                         | States                            | `one2many`  |     | ✅    | res.country.state |
| `state_required`                    | State Required                    | `boolean`   |     | ✅    |                   |
| `vat_label`                         | Vat Label                         | `char`      |     | ✅    |                   |
| `write_date`                        | Last Updated on                   | `datetime`  |     | ✅    |                   |
| `write_uid`                         | Last Updated by                   | `many2one`  |     | ✅    | res.users         |
...
| Name                        | Group                | Perms     |
| --------------------------- | -------------------- | --------- |
| name_op_res_country_student | OpenEduCat / Manager | `R/W/C/D` |
| name_op_res_country_faculty | OpenEduCat / User    | `R`       |
| res_country group_user      | Contact / Creation   | `R`       |
| res_country_group_system    | Role / Administrator | `R/W/C/D` |
| name_op_res_country_parent  | Role / Portal        | `R`       |
| res_country group_user_all  | Role / Portal        | `R`       |
| res_country group_user_all  | Role / Public        | `R`       |
| res_country group_user_all  | Role / User          | `R`       |
...
### 🗃️ `payment.capture.wizard` — Payment Capture Wizard
...
| Field                        | Label                      | Type        | Req | Store | Relation            |
| ---------------------------- | -------------------------- | ----------- | --- | ----- | ------------------- |
| `amount_to_capture`          | Amount To Capture          | `monetary`  |     | ✅    |                     |
| `authorized_amount`          | Authorized Amount          | `monetary`  |     |       |                     |
| `available_amount`           | Maximum Capture Allowed    | `monetary`  |     |       |                     |
| `captured_amount`            | Already Captured           | `monetary`  |     |       |                     |
| `create_date`                | Created on                 | `datetime`  |     | ✅    |                     |
| `create_uid`                 | Created by                 | `many2one`  |     | ✅    | res.users           |
| `currency_id`                | Currency                   | `many2one`  |     |       | res.currency        |
| `display_name`               | Display Name               | `char`      |     |       |                     |
| `has_draft_children`         | Has Draft Children         | `boolean`   |     |       |                     |
| `has_remaining_amount`       | Has Remaining Amount       | `boolean`   |     |       |                     |
| `id`                         | ID                         | `integer`   |     | ✅    |                     |
| `is_amount_to_capture_valid` | Is Amount To Capture Valid | `boolean`   |     |       |                     |
| `support_partial_capture`    | Support Partial Capture    | `boolean`   |     |       |                     |
| `transaction_ids`            | Transaction                | `many2many` |     | ✅    | payment.transaction |
| `voided_amount`              | Already Voided             | `monetary`  |     |       |                     |
...
| Name                   | Group       | Perms   |
| ---------------------- | ----------- | ------- |
| payment.capture.wizard | Role / User | `R/W/C` |

**Record Rules:**

- **Payment Capture Wizard** (Global) `R/W/C/D`
  - Domain: `[('create_uid', '=', user.id)]`

### 🗃️ `payment.method` — Payment Method
...
| --------------------------- | ------------------------------------------------ | ----------- | --- | ----- | ---------------- |
| `active`                    | Active                                           | `boolean`   |     | ✅    |                  |
| `brand_ids`                 | Brands                                           | `one2many`  |     | ✅    | payment.method   |
| `code`                      | Code                                             | `char`      | ✅  | ✅    |                  |
| `create_date`               | Created on                                       | `datetime`  |     | ✅    |                  |
| `create_uid`                | Created by                                       | `many2one`  |     | ✅    | res.users        |
| `display_name`              | Display Name                                     | `char`      |     |       |                  |
| `id`                        | ID                                               | `integer`   |     | ✅    |                  |
| `image`                     | Image                                            | `binary`    | ✅  | ✅    |                  |
| `image_payment_form`        | The resized image displayed on the payment form. | `binary`    |     | ✅    |                  |
| `is_primary`                | Is Primary Payment Method                        | `boolean`   |     |       |                  |
| `name`                      | Name                                             | `char`      | ✅  | ✅    |                  |
| `primary_payment_method_id` | Primary Payment Method                           | `many2one`  |     | ✅    | payment.method   |
| `provider_ids`              | Providers                                        | `many2many` |     | ✅    | payment.provider |
| `sequence`                  | Sequence                                         | `integer`   |     | ✅    |                  |
| `supported_country_ids`     | Countries                                        | `many2many` |     | ✅    | res.country      |
| `supported_currency_ids`    | Currencies                                       | `many2many` |     | ✅    | res.currency     |
| `support_express_checkout`  | Express Checkout                                 | `boolean`   |     | ✅    |                  |
| `support_manual_capture`    | Manual Capture                                   | `selection` | ✅  | ✅    |                  |
...
| Name                  | Group                | Perms     |
| --------------------- | -------------------- | --------- |
| payment.method.system | Role / Administrator | `R/W/C/D` |
| payment.method.all    | Role / Portal        | `R`       |
| payment.method.all    | Role / Public        | `R`       |
| payment.method.all    | Role / User          | `R`       |
...
| `create_date`          | Created on           | `datetime`  |     | ✅    |                     |
| `create_uid`           | Created by           | `many2one`  |     | ✅    | res.users           |
| `demo_simulated_state` | Simulated State      | `selection` |     | ✅    |                     |
| `display_name`         | Display Name         | `char`      |     |       |                     |
| `id`                   | ID                   | `integer`   |     | ✅    |                     |
| `partner_id`           | Partner              | `many2one`  | ✅  | ✅    | res.partner         |
| `payment_details`      | Payment Details      | `char`      |     | ✅    |                     |
...
| `provider_ref`         | Provider Reference   | `char`      | ✅  | ✅    |                     |
| `transaction_ids`      | Payment Transactions | `one2many`  |     | ✅    | payment.transaction |
| `write_date`           | Last Updated on      | `datetime`  |     | ✅    |                     |
| `write_uid`            | Last Updated by      | `many2one`  |     | ✅    | res.users           |
...
| Name                 | Group                | Perms     |
| -------------------- | -------------------- | --------- |
| payment.token.system | Role / Administrator | `R/W/C/D` |
| payment.token.all    | Role / Portal        | `R`       |
| payment.token.all    | Role / Public        | `R`       |
| payment.token.all    | Role / User          | `R`       |

**Record Rules:**

- **Users can access only their own tokens** (10, 11, 1) `R/W/C/D`
  - Domain: `[('partner_id', '=', user.partner_id.id)]`
- **Access tokens in own companies only** (Global) `R/W/C/D`
  - Domain: `[('company_id', 'parent_of', company_ids)]`
- **Access every token** (37) `R/W/C/D`
  - Domain: `[(1, '=', 1)]`
- **Access every payment token** (25) `R/W/C/D`
  - Domain: `[(1, '=', 1)]`
