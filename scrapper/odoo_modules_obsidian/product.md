...
> **Module:** `product` | **Version:** `19.0.1.2` **Category:** Sales | **License:**
...
Products support variants, different pricing methods, vendors information, make to
stock/order, different units of measure, packaging and properties.

## Pricelists support:

> - Multiple-level of discount (by product, category, quantities)
> - Compute price based on different criteria: : - Other pricelist - Cost price - List
>   price - Vendor price
...
- `* INHERIT product.pricelist.item.product.product.form.inherit (form)`
- `* INHERIT product.pricelist.item.product.template.form.inherit (form)`
- `* INHERIT product.product.form (form)`
- `* INHERIT product.product.search (search)`
- `* INHERIT product.template.list.purchasable (list)`
- `* INHERIT product.template.list.sellable (list)`
...
- `report_simple_label_dymo (qweb)`
- `update.product.attribute.value.form (form)`

### Reports

- `Packaging Barcodes (PDF)`
- `Pricelist`
- `Product Label (PDF)`
...
**33 model(s) defined by this module:**
...
| `activity_type_id`              | Next Activity Type            | `many2one`  |     |       | mail.activity.type     |
| `activity_user_id`              | Responsible User              | `many2one`  |     |       | res.users              |
| `code`                          | E-commerce Promotional Code   | `char`      |     | ✅    |                        |
| `company_id`                    | Company                       | `many2one`  |     | ✅    | res.company            |
| `country_group_ids`             | Country Groups                | `many2many` |     | ✅    | res.country.group      |
| `create_date`                   | Created on                    | `datetime`  |     | ✅    |                        |
| `create_uid`                    | Created by                    | `many2one`  |     | ✅    | res.users              |
| `currency_id`                   | Currency                      | `many2one`  | ✅  | ✅    | res.currency           |
| `display_name`                  | Display Name                  | `char`      |     |       |                        |
| `has_message`                   | Has Message                   | `boolean`   |     |       |                        |
| `id`                            | ID                            | `integer`   |     | ✅    |                        |
| `item_ids`                      | Pricelist Rules               | `one2many`  |     | ✅    | product.pricelist.item |
| `message_attachment_count`      | Attachment Count              | `integer`   |     |       |                        |
| `message_follower_ids`          | Followers                     | `one2many`  |     | ✅    | mail.followers         |
| `message_has_error`             | Message Delivery error        | `boolean`   |     |       |                        |
| `message_has_error_counter`     | Number of errors              | `integer`   |     |       |                        |
...
| Name                              | Group                            | Perms     |
| --------------------------------- | -------------------------------- | --------- |
| product.pricelist.sale.user       | Sales / User: Own Documents Only | `R`       |
| product.pricelist salemanager     | Sales / Administrator            | `R/W/C/D` |
| product.pricelist.manager         | Products / Create                | `R/W/C/D` |
| product.pricelist partner manager | Contact / Creation               | `R`       |
| product.pricelist stock_manager   | Inventory / Administrator        | `R/W/C/D` |
| product.pricelist.public          | Role / Portal                    | `R`       |
| product.pricelist.public          | Role / Public                    | `R`       |
| product.pricelist.public          | Role / User                      | `R`       |
| product.pricelist.user            | Role / User                      | `R`       |

**Record Rules:**

- **product pricelist company rule** (Global) `R/W/C/D`
  - Domain:
    `['|', ('company_id', 'in', [False, website.company_id.id]), ('company_id', 'in', company_ids)]`
...
### 🗃️ `product.attribute.value` — Attribute Value
...
| ----------------------------- | --------------------------- | ----------- | --- | ----- | ------------------------------- |
| `active`                      | Active                      | `boolean`   |     | ✅    |                                 |
| `attribute_id`                | Attribute                   | `many2one`  | ✅  | ✅    | product.attribute               |
| `color`                       | Color Index                 | `integer`   |     | ✅    |                                 |
| `create_date`                 | Created on                  | `datetime`  |     | ✅    |                                 |
| `create_uid`                  | Created by                  | `many2one`  |     | ✅    | res.users                       |
| `default_extra_price`         | Default Extra Price         | `float`     |     | ✅    |                                 |
| `default_extra_price_changed` | Default Extra Price Changed | `boolean`   |     |       |                                 |
| `display_name`                | Display Name                | `char`      |     |       |                                 |
| `display_type`                | Display Type                | `selection` |     |       |                                 |
| `html_color`                  | Color                       | `char`      |     | ✅    |                                 |
| `id`                          | ID                          | `integer`   |     | ✅    |                                 |
| `image`                       | Image                       | `binary`    |     | ✅    |                                 |
| `is_custom`                   | Free text                   | `boolean`   |     | ✅    |                                 |
| `is_used_on_products`         | Used on Products            | `boolean`   |     |       |                                 |
| `name`                        | Value                       | `char`      | ✅  | ✅    |                                 |
| `pav_attribute_line_ids`      | Lines                       | `many2many` |     | ✅    | product.template.attribute.line |
| `sequence`                    | Sequence                    | `integer`   |     | ✅    |                                 |
| `write_date`                  | Last Updated on             | `datetime`  |     | ✅    |                                 |
| `write_uid`                   | Last Updated by             | `many2one`  |     | ✅    | res.users                       |
...
| Name                            | Group                     | Perms     |
| ------------------------------- | ------------------------- | --------- |
| product.attribute value manager | Products / Create         | `R/W/C/D` |
| product.attribute manager value | Inventory / Administrator | `R/W/C/D` |
| product.attribute value public  | Role / Portal             | `R`       |
| product.attribute value public  | Role / Public             | `R`       |
| product.attribute value         | Role / User               | `R`       |
| product.attribute value public  | Role / User               | `R`       |

### 🗃️ `product.label.layout` — Choose the sheet layout to print the labels
...
| `create_date`      | Created on        | `datetime`  |     | ✅    |                   |
| `create_uid`       | Created by        | `many2one`  |     | ✅    | res.users         |
| `custom_quantity`  | Copies            | `integer`   | ✅  | ✅    |                   |
| `display_name`     | Display Name      | `char`      |     |       |                   |
| `extra_html`       | Extra Content     | `html`      |     | ✅    |                   |
| `id`               | ID                | `integer`   |     | ✅    |                   |
| `move_ids`         | Move              | `many2many` |     | ✅    | stock.move        |
| `move_quantity`    | Quantity to print | `selection` | ✅  | ✅    |                   |
| `pricelist_id`     | Pricelist         | `many2one`  |     | ✅    | product.pricelist |
| `print_format`     | Format            | `selection` | ✅  | ✅    |                   |
| `product_ids`      | Product           | `many2many` |     | ✅    | product.product   |
| `product_tmpl_ids` | Product Tmpl      | `many2many` |     | ✅    | product.template  |
| `rows`             | Rows              | `integer`   |     |       |                   |
| `write_date`       | Last Updated on   | `datetime`  |     | ✅    |                   |
| `write_uid`        | Last Updated by   | `many2one`  |     | ✅    | res.users         |
| `zpl_preview`      | ZPL Preview       | `binary`    |     | ✅    |                   |
| `zpl_template`     | ZPL Template      | `selection` | ✅  | ✅    |                   |

**Access Rights:**

| Name                      | Group       | Perms     |
| ------------------------- | ----------- | --------- |
| product.label.layout.user | Role / User | `R/W/C/D` |
...
### 🗃️ `product.uom` — Link between products and their UoMs
...
| Name                | Group             | Perms     |
| ------------------- | ----------------- | --------- |
| product.uom manager | Products / Create | `R/W/C/D` |
| product.uom user    | Role / User       | `R`       |

### 🗃️ `report.product.report_pricelist` — Pricelist Report
...
### 🗃️ `product.pricelist.item` — Pricelist Rule
...
| Field                   | Label                 | Type        | Req | Store | Relation          |
| ----------------------- | --------------------- | ----------- | --- | ----- | ----------------- |
| `applied_on`            | Apply On              | `selection` | ✅  | ✅    |                   |
| `base`                  | Based on              | `selection` | ✅  | ✅    |                   |
| `base_pricelist_id`     | Other Pricelist       | `many2one`  |     | ✅    | product.pricelist |
| `categ_id`              | Category              | `many2one`  |     | ✅    | product.category  |
| `company_id`            | Company               | `many2one`  |     | ✅    | res.company       |
| `compute_price`         | Compute Price         | `selection` | ✅  | ✅    |                   |
| `create_date`           | Created on            | `datetime`  |     | ✅    |                   |
| `create_uid`            | Created by            | `many2one`  |     | ✅    | res.users         |
| `currency_id`           | Currency              | `many2one`  |     | ✅    | res.currency      |
| `date_end`              | End Date              | `datetime`  |     | ✅    |                   |
| `date_start`            | Start Date            | `datetime`  |     | ✅    |                   |
| `display_applied_on`    | Display Applied On    | `selection` | ✅  | ✅    |                   |
| `display_name`          | Display Name          | `char`      |     |       |                   |
| `fixed_price`           | Fixed Price           | `float`     |     | ✅    |                   |
| `id`                    | ID                    | `integer`   |     | ✅    |                   |
| `is_pricelist_required` | Is Pricelist Required | `boolean`   |     |       |                   |
| `min_quantity`          | Min. Quantity         | `float`     |     | ✅    |                   |
| `name`                  | Name                  | `char`      |     |       |                   |
| `percent_price`         | Percentage Price      | `float`     |     | ✅    |                   |
| `price`                 | Price                 | `char`      |     |       |                   |
| `price_discount`        | Price Discount        | `float`     |     | ✅    |                   |
| `pricelist_id`          | Pricelist             | `many2one`  |     | ✅    | product.pricelist |
| `price_markup`          | Markup                | `float`     |     | ✅    |                   |
| `price_max_margin`      | Max. Price Margin     | `float`     |     | ✅    |                   |
| `price_min_margin`      | Min. Price Margin     | `float`     |     | ✅    |                   |
| `price_round`           | Price Rounding        | `float`     |     | ✅    |                   |
| `price_surcharge`       | Extra Fee             | `float`     |     | ✅    |                   |
| `product_id`            | Variant               | `many2one`  |     | ✅    | product.product   |
| `product_tmpl_id`       | Product               | `many2one`  |     | ✅    | product.template  |
| `product_uom_name`      | Unit Name             | `char`      |     |       |                   |
| `product_variant_count` | # Product Variants    | `integer`   |     |       |                   |
| `rule_tip`              | Rule Tip              | `char`      |     |       |                   |
| `write_date`            | Last Updated on       | `datetime`  |     | ✅    |                   |
| `write_uid`             | Last Updated by       | `many2one`  |     | ✅    | res.users         |
...
| Name                                    | Group                    | Perms     |
| --------------------------------------- | ------------------------ | --------- |
| product.pricelist.item salemanager      | Sales / Administrator    | `R/W/C/D` |
| product.pricelist.item purchase_manager | Purchase / Administrator | `R/W/C/D` |
| product.pricelist.item.manager          | Products / Create        | `R/W/C/D` |
| product.pricelist.item.public           | Role / Portal            | `R`       |
| product.pricelist.item.public           | Role / Public            | `R`       |
| product.pricelist.item.public           | Role / User              | `R`       |
| product.pricelist.item.user             | Role / User              | `R`       |

**Record Rules:**

- **product pricelist item company rule** (Global) `R/W/C/D`
  - Domain:
    `['|', ('company_id', 'in', [False, website.company_id.id]), ('company_id', 'in', company_ids)]`

### 🗃️ `product.attribute` — Product Attribute
...
| ------------------------- | ----------------------- | ----------- | --- | ----- | -------------------------------- |
| `active`                  | Active                  | `boolean`   |     | ✅    |                                  |
| `attribute_line_ids`      | Lines                   | `one2many`  |     | ✅    | product.template.attribute.line  |
| `category_id`             | eCommerce Category      | `many2one`  |     | ✅    | product.attribute.category       |
| `create_date`             | Created on              | `datetime`  |     | ✅    |                                  |
| `create_uid`              | Created by              | `many2one`  |     | ✅    | res.users                        |
| `create_variant`          | Variant Creation        | `selection` | ✅  | ✅    |                                  |
| `display_name`            | Display Name            | `char`      |     |       |                                  |
| `display_type`            | Display Type            | `selection` | ✅  | ✅    |                                  |
| `id`                      | ID                      | `integer`   |     | ✅    |                                  |
| `is_thumbnail_visible`    | Show Thumbnails         | `boolean`   |     | ✅    |                                  |
| `name`                    | Attribute               | `char`      | ✅  | ✅    |                                  |
| `number_related_products` | Number Related Products | `integer`   |     |       |                                  |
| `preview_variants`        | On Product Cards        | `selection` |     | ✅    |                                  |
| `product_tmpl_ids`        | Related Products        | `many2many` |     | ✅    | product.template                 |
| `sequence`                | Sequence                | `integer`   |     | ✅    |                                  |
| `template_value_ids`      | Template Values         | `one2many`  |     | ✅    | product.template.attribute.value |
| `value_ids`               | Values                  | `one2many`  |     | ✅    | product.attribute.value          |
...
| Name                      | Group                     | Perms     |
| ------------------------- | ------------------------- | --------- |
| product.attribute manager | Products / Create         | `R/W/C/D` |
| product.attribute manager | Inventory / Administrator | `R/W/C/D` |
| product.attribute public  | Role / Portal             | `R`       |
| product.attribute public  | Role / Public             | `R`       |
| product.attribute         | Role / User               | `R`       |
| product.attribute public  | Role / User               | `R`       |

### 🗃️ `product.attribute.custom.value` — Product Attribute Custom Value
...
| `create_date`                                | Created on       | `datetime` |     | ✅    |                                  |
| `create_uid`                                 | Created by       | `many2one` |     | ✅    | res.users                        |
| `custom_product_template_attribute_value_id` | Attribute Value  | `many2one` | ✅  | ✅    | product.template.attribute.value |
...
| Name                                    | Group                            | Perms     |
| --------------------------------------- | -------------------------------- | --------- |
| product.attribute.custom value salesman | Sales / User: Own Documents Only | `R/W/C/D` |
| product.attribute.custom value manager  | Products / Create                | `R/W/C/D` |
| product.attribute.custom value          | Role / Portal                    | `R`       |
| product.attribute.custom value          | Role / Public                    | `R`       |
| product.attribute.custom value          | Role / User                      | `R`       |
| product.attribute.custom value manager  | Role / User                      | `R`       |
...
### 🗃️ `product.combo.item` — Product Combo Item
...
| Field          | Label           | Type       | Req | Store | Relation        |
| -------------- | --------------- | ---------- | --- | ----- | --------------- |
| `combo_id`     | Combo           | `many2one` | ✅  | ✅    | product.combo   |
| `company_id`   | Company         | `many2one` |     | ✅    | res.company     |
| `create_date`  | Created on      | `datetime` |     | ✅    |                 |
| `create_uid`   | Created by      | `many2one` |     | ✅    | res.users       |
| `currency_id`  | Currency        | `many2one` |     |       | res.currency    |
| `display_name` | Display Name    | `char`     |     |       |                 |
| `extra_price`  | Extra Price     | `float`    |     | ✅    |                 |
| `id`           | ID              | `integer`  |     | ✅    |                 |
| `lst_price`    | Original Price  | `float`    |     |       |                 |
| `product_id`   | Options         | `many2one` | ✅  | ✅    | product.product |
| `write_date`   | Last Updated on | `datetime` |     | ✅    |                 |
| `write_uid`    | Last Updated by | `many2one` |     | ✅    | res.users       |
...
| Name                       | Group             | Perms     |
| -------------------------- | ----------------- | --------- |
| product.combo.item.manager | Products / Create | `R/W/C/D` |
| product.combo.item.user    | Role / User       | `R`       |
...
### 🗃️ `report.product.report_producttemplatelabel_dymo` — Product Label Report
...
### 🗃️ `report.product.report_producttemplatelabel2x7` — Product Label Report 2x7
...
### 🗃️ `report.product.report_producttemplatelabel4x12` — Product Label Report 4x12
...
### 🗃️ `report.product.report_producttemplatelabel4x12noprice` — Product Label Report 4x12 No Price
...
### 🗃️ `report.product.report_producttemplatelabel4x7` — Product Label Report 4x7
...
| `image`                | Image                               | `binary`    |     | ✅    |                  |
| `name`                 | Name                                | `char`      | ✅  | ✅    |                  |
| `product_ids`          | All Product Variants using this Tag | `many2many` |     |       | product.product  |
| `product_product_ids`  | Product Variants                    | `many2many` |     | ✅    | product.product  |
| `product_template_ids` | Product Templates                   | `many2many` |     | ✅    | product.template |
| `sequence`             | Sequence                            | `integer`   |     | ✅    |                  |
| `visible_to_customers` | Visible to customers                | `boolean`   |     | ✅    |                  |
| `website_id`           | Website                             | `many2one`  |     | ✅    | website          |
| `write_date`           | Last Updated on                     | `datetime`  |     | ✅    |                  |
| `write_uid`            | Last Updated by                     | `many2one`  |     | ✅    | res.users        |
...
| Name                | Group             | Perms     |
| ------------------- | ----------------- | --------- |
| product.tag.manager | Products / Create | `R/W/C/D` |
| product.tag.public  | Role / Portal     | `R`       |
| product.tag.public  | Role / Public     | `R`       |
| product.tag.public  | Role / User       | `R`       |
| product.tag.user    | Role / User       | `R`       |

### 🗃️ `product.template.attribute.exclusion` — Product Template Attribute Exclusion
...
| `display_name`                        | Display Name     | `char`      |     |       |                                  |
| `id`                                  | ID               | `integer`   |     | ✅    |                                  |
| `product_template_attribute_value_id` | Attribute Value  | `many2one`  |     | ✅    | product.template.attribute.value |
| `product_tmpl_id`                     | Product Template | `many2one`  | ✅  | ✅    | product.template                 |
| `value_ids`                           | Attribute Values | `many2many` |     | ✅    | product.template.attribute.value |
| `write_date`                          | Last Updated on  | `datetime`  |     | ✅    |                                  |
| `write_uid`                           | Last Updated by  | `many2one`  |     | ✅    | res.users                        |
...
| Name                                         | Group             | Perms     |
| -------------------------------------------- | ----------------- | --------- |
| product.template.attribute exclusion manager | Products / Create | `R/W/C/D` |
| product.template.attribute exclusion public  | Role / Portal     | `R`       |
| product.template.attribute exclusion public  | Role / Public     | `R`       |
| product..template.attribute exclusion        | Role / User       | `R`       |
| product.template.attribute exclusion public  | Role / User       | `R`       |

### 🗃️ `product.template.attribute.line` — Product Template Attribute Line

Attributes available on product.template with their selected values in a m2m. Used as a
configuration model to generate the appropriate product.template.attribute.value
...
| `display_name`               | Display Name             | `char`      |     |       |                                  |
| `id`                         | ID                       | `integer`   |     | ✅    |                                  |
| `product_template_value_ids` | Product Attribute Values | `one2many`  |     | ✅    | product.template.attribute.value |
| `product_tmpl_id`            | Product Template         | `many2one`  | ✅  | ✅    | product.template                 |
| `sequence`                   | Sequence                 | `integer`   |     | ✅    |                                  |
| `value_count`                | Value Count              | `integer`   |     | ✅    |                                  |
| `value_ids`                  | Values                   | `many2many` |     | ✅    | product.attribute.value          |
| `write_date`                 | Last Updated on          | `datetime`  |     | ✅    |                                  |
| `write_uid`                  | Last Updated by          | `many2one`  |     | ✅    | res.users                        |
...
| Name                                    | Group             | Perms     |
| --------------------------------------- | ----------------- | --------- |
| product.template.attribute line manager | Products / Create | `R/W/C/D` |
| product.template.attribute line public  | Role / Portal     | `R`       |
| product.template.attribute line public  | Role / Public     | `R`       |
| product.template.attribute line         | Role / User       | `R`       |
| product.template.attribute line public  | Role / User       | `R`       |

### 🗃️ `product.template.attribute.value` — Product Template Attribute Value

Materialized relationship between attribute values and product template generated by the
...
| ---------------------------- | ---------------- | ----------- | --- | ----- | ------------------------------------ |
| `attribute_id`               | Attribute        | `many2one`  |     | ✅    | product.attribute                    |
| `attribute_line_id`          | Attribute Line   | `many2one`  | ✅  | ✅    | product.template.attribute.line      |
| `color`                      | Color            | `integer`   |     | ✅    |                                      |
| `create_date`                | Created on       | `datetime`  |     | ✅    |                                      |
| `create_uid`                 | Created by       | `many2one`  |     | ✅    | res.users                            |
| `currency_id`                | Currency         | `many2one`  |     |       | res.currency                         |
| `display_name`               | Display Name     | `char`      |     |       |                                      |
| `display_type`               | Display Type     | `selection` |     |       |                                      |
| `exclude_for`                | Exclude for      | `one2many`  |     | ✅    | product.template.attribute.exclusion |
| `html_color`                 | HTML Color Index | `char`      |     |       |                                      |
| `id`                         | ID               | `integer`   |     | ✅    |                                      |
| `image`                      | Image            | `binary`    |     |       |                                      |
| `is_custom`                  | Free text        | `boolean`   |     |       |                                      |
| `name`                       | Value            | `char`      |     |       |                                      |
| `price_extra`                | Extra Price      | `float`     |     | ✅    |                                      |
| `product_attribute_value_id` | Attribute Value  | `many2one`  | ✅  | ✅    | product.attribute.value              |
| `product_tmpl_id`            | Product Template | `many2one`  |     | ✅    | product.template                     |
| `ptav_active`                | Active           | `boolean`   |     | ✅    |                                      |
| `ptav_product_variant_ids`   | Related Variants | `many2many` |     | ✅    | product.product                      |
| `write_date`                 | Last Updated on  | `datetime`  |     | ✅    |                                      |
| `write_uid`                  | Last Updated by  | `many2one`  |     | ✅    | res.users                            |
...
| Name                                     | Group             | Perms     |
| ---------------------------------------- | ----------------- | --------- |
| product.template.attribute value manager | Products / Create | `R/W/C/D` |
| product.template.attribute value public  | Role / Portal     | `R`       |
| product.template.attribute value public  | Role / Public     | `R`       |
| product.template.attribute value         | Role / User       | `R`       |
| product.template.attribute value public  | Role / User       | `R`       |
...
### 🗃️ `update.product.attribute.value` — Update product attribute value
...
| Field                | Label           | Type        | Req | Store | Relation                |
| -------------------- | --------------- | ----------- | --- | ----- | ----------------------- |
| `attribute_value_id` | Attribute Value | `many2one`  | ✅  | ✅    | product.attribute.value |
| `create_date`        | Created on      | `datetime`  |     | ✅    |                         |
| `create_uid`         | Created by      | `many2one`  |     | ✅    | res.users               |
| `display_name`       | Display Name    | `char`      |     |       |                         |
| `id`                 | ID              | `integer`   |     | ✅    |                         |
...
| `mode`               | Mode            | `selection` |     | ✅    |                         |
| `product_count`      | Product Count   | `integer`   |     |       |                         |
| `write_date`         | Last Updated on | `datetime`  |     | ✅    |                         |
| `write_uid`          | Last Updated by | `many2one`  |     | ✅    | res.users               |
...
| Name                           | Group                     | Perms   |
| ------------------------------ | ------------------------- | ------- |
| update.product.attribute.value | Products / Create         | `R/W/C` |
| update.product.attribute.value | Inventory / Administrator | `R/W/C` |
