...
> **Module:** `event_sale` | **Version:** `19.0.1.3` **Category:** Events | **License:**
> `LGPL-3` **Author:** Odoo S.A. **Website:** https://www.odoo.com/app/events

## 🔗 Dependencies

[[event_product]] [[sale_management]]

## 📖 Description

# Creating registration with sales orders.

This module allows you to automate and connect your registration creation with your main
sale flow and therefore, to enable the invoicing feature of registrations.

It defines a new kind of service products that offers you the possibility to choose an
event category associated with it. When you encode a sales order for that product, you
will be able to choose an existing event of that category and when you confirm your
sales order it will automatically create a registration for this event.

## 🖥️ UI Components

### Menus

- `Events/Reporting/Revenues`
...
### 🗃️ `registration.editor` — Edit Attendee Details on Sales Confirmation
...
| `create_uid`             | Created by            | `many2one` |     | ✅    | res.users                |
| `display_name`           | Display Name          | `char`     |     |       |                          |
| `event_registration_ids` | Registrations to Edit | `one2many` |     | ✅    | registration.editor.line |
| `id`                     | ID                    | `integer`  |     | ✅    |                          |
| `sale_order_id`          | Sales Order           | `many2one` | ✅  | ✅    | sale.order               |
| `write_date`             | Last Updated on       | `datetime` |     | ✅    |                          |
| `write_uid`              | Last Updated by       | `many2one` |     | ✅    | res.users                |
...
| Name                       | Group                            | Perms   |
| -------------------------- | -------------------------------- | ------- |
| access.registration.editor | Sales / User: Own Documents Only | `R/W/C` |

### 🗃️ `registration.editor.line` — Edit Attendee Line on Sales Confirmation
...
| `create_uid`         | Created by            | `many2one` |     | ✅    | res.users           |
| `display_name`       | Display Name          | `char`     |     |       |                     |
| `editor_id`          | Editor                | `many2one` |     | ✅    | registration.editor |
| `email`              | Email                 | `char`     |     | ✅    |                     |
| `event_id`           | Event                 | `many2one` | ✅  | ✅    | event.event         |
...
| `name`               | Name                  | `char`     |     | ✅    |                     |
| `phone`              | Phone                 | `char`     |     | ✅    |                     |
| `registration_id`    | Original Registration | `many2one` |     | ✅    | event.registration  |
| `sale_order_line_id` | Sales Order Line      | `many2one` |     | ✅    | sale.order.line     |
| `write_date`         | Last Updated on       | `datetime` |     | ✅    |                     |
| `write_uid`          | Last Updated by       | `many2one` |     | ✅    | res.users           |
...
| Name                            | Group                            | Perms     |
| ------------------------------- | -------------------------------- | --------- |
| access.registration.editor.line | Sales / User: Own Documents Only | `R/W/C/D` |

### 🗃️ `event.event.configurator` — Event Configurator
...
| `event_slot_id`         | Slot                  | `many2one` |     | ✅    | event.slot         |
| `event_ticket_id`       | Ticket Type           | `many2one` |     | ✅    | event.event.ticket |
| `has_available_tickets` | Has Available Tickets | `boolean`  |     |       |                    |
| `id`                    | ID                    | `integer`  |     | ✅    |                    |
| `is_multi_slots`        | Is Multi Slots        | `boolean`  |     |       |                    |
...
| Name                            | Group                            | Perms   |
| ------------------------------- | -------------------------------- | ------- |
| access.event.event.configurator | Sales / User: Own Documents Only | `R/W/C` |
