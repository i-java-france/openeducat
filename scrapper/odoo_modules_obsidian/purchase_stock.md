...
> **Module:** `purchase_stock` | **Version:** `19.0.1.2` **Category:** Purchase |
...
- `* INHERIT Stock Warehouse Inherited (form)`
- `* INHERIT product.replenish.form.inherit.stock (form)`
- `* INHERIT product.supplierinfo.replenishment.list.view (list)`
- `* INHERIT product.view.kanban.catalog.purchase_stock (kanban)`
- `* INHERIT purchase.order.form.inherit (form)`
- `* INHERIT purchase.order.line.form.inherit (form)`
- `* INHERIT purchase.order.list.inherit (list)`
- `* INHERIT purchase.report.search.stock (search)`
- `* INHERIT purchase.stock.view.picking.form (form)`
- `* INHERIT purchase.view.search.catalog.inherit.purchase_stock (search)`
- `* INHERIT purchase_help_message_template (qweb)`
- `* INHERIT purchase_report_stock_rule (qweb)`
- `* INHERIT report_purchaseorder_document (qweb)`
- `* INHERIT report_purchasequotation_document (qweb)`
- `* INHERIT res.config.settings.view.form.inherit.purchase (form)`
- `* INHERIT res.config.settings.view.form.inherit.purchase.stock (form)`
- `* INHERIT res.partner.purchase.stock.form.inherit (form)`
- `* INHERIT stock.move.form (form)`
- `* INHERIT stock.production.lot.view.form (form)`
- `* INHERIT stock.reference.purchase (form)`
- `* INHERIT stock.replenishment.information.purchase.stock.inherit (form)`
- `* INHERIT stock.rule.form.stock.inherit.purchase_stock (form)`
- `* INHERIT stock.warehouse.orderpoint.list.editable.inherit.mrp (list)`
- `* INHERIT stock.warehouse.orderpoint.search.inherit (search)`
- `exception_on_po (qweb)`
- `vendor.delay.report.search (search)`
- `vendor.delay.report.view.graph (graph)`
...
### 🗃️ `stock.warehouse.orderpoint` — Minimum Inventory Rule
...
| ---------------------------------- | -------------------------------- | ----------- | --- | ----- | -------------------- |
| `active`                           | Active                           | `boolean`   |     | ✅    |                      |
| `allowed_location_ids`             | Allowed Location                 | `one2many`  |     |       | stock.location       |
| `allowed_replenishment_uom_ids`    | Allowed Replenishment Uom        | `many2many` |     |       | uom.uom              |
| `available_vendor`                 | Available Vendor                 | `many2one`  |     |       | res.partner          |
| `company_id`                       | Company                          | `many2one`  | ✅  | ✅    | res.company          |
| `create_date`                      | Created on                       | `datetime`  |     | ✅    |                      |
| `create_uid`                       | Created by                       | `many2one`  |     | ✅    | res.users            |
| `days_to_order`                    | Days To Order                    | `float`     |     |       |                      |
| `deadline_date`                    | Deadline                         | `date`      |     | ✅    |                      |
| `display_name`                     | Display Name                     | `char`      |     |       |                      |
| `effective_route_id`               | Effective Route                  | `many2one`  |     |       | stock.route          |
| `effective_vendor_id`              | Effective Vendor                 | `many2one`  |     |       | res.partner          |
| `id`                               | ID                               | `integer`   |     | ✅    |                      |
| `lead_days`                        | Lead Days                        | `float`     |     |       |                      |
| `lead_horizon_date`                | Lead Horizon Date                | `date`      |     |       |                      |
| `location_id`                      | Location                         | `many2one`  | ✅  | ✅    | stock.location       |
| `name`                             | Name                             | `char`      | ✅  | ✅    |                      |
| `product_category_id`              | Product Category                 | `many2one`  |     |       | product.category     |
| `product_id`                       | Product                          | `many2one`  | ✅  | ✅    | product.product      |
| `product_max_qty`                  | Max Quantity                     | `float`     | ✅  | ✅    |                      |
| `product_min_qty`                  | Min Quantity                     | `float`     | ✅  | ✅    |                      |
| `product_tmpl_id`                  | Product Template                 | `many2one`  |     |       | product.template     |
| `product_uom`                      | Unit                             | `many2one`  |     |       | uom.uom              |
| `product_uom_name`                 | Product unit of measure label    | `char`      |     |       |                      |
| `qty_forecast`                     | Forecast                         | `float`     |     |       |                      |
| `qty_on_hand`                      | On Hand                          | `float`     |     |       |                      |
| `qty_to_order`                     | To Order                         | `float`     |     |       |                      |
| `qty_to_order_computed`            | To Order Computed                | `float`     |     | ✅    |                      |
| `qty_to_order_manual`              | To Order Manual                  | `float`     |     | ✅    |                      |
| `replenishment_uom_id`             | Multiple                         | `many2one`  |     | ✅    | uom.uom              |
| `replenishment_uom_id_placeholder` | Replenishment Uom Id Placeholder | `char`      |     |       |                      |
| `route_id`                         | Route                            | `many2one`  |     | ✅    | stock.route          |
| `route_id_placeholder`             | Route Id Placeholder             | `char`      |     |       |                      |
| `rule_ids`                         | Rules used                       | `many2many` |     |       | stock.rule           |
| `show_supplier`                    | Show supplier column             | `boolean`   |     |       |                      |
| `show_supply_warning`              | Show Supply Warning              | `boolean`   |     |       |                      |
| `snoozed_until`                    | Snoozed                          | `date`      |     | ✅    |                      |
| `supplier_id`                      | Vendor Pricelist                 | `many2one`  |     | ✅    | product.supplierinfo |
| `supplier_id_placeholder`          | Supplier Id Placeholder          | `char`      |     |       |                      |
| `trigger`                          | Trigger                          | `selection` | ✅  | ✅    |                      |
| `unwanted_replenish`               | Unwanted Replenish               | `boolean`   |     |       |                      |
| `vendor_ids`                       | Vendors                          | `one2many`  |     |       | product.supplierinfo |
| `warehouse_id`                     | Warehouse                        | `many2one`  | ✅  | ✅    | stock.warehouse      |
| `write_date`                       | Last Updated on                  | `datetime`  |     | ✅    |                      |
| `write_uid`                        | Last Updated by                  | `many2one`  |     | ✅    | res.users            |
...
| Name                              | Group                            | Perms     |
| --------------------------------- | -------------------------------- | --------- |
| stock.warehouse.orderpoint        | Sales / User: Own Documents Only | `R`       |
| stock.warehouse.orderpoint        | Purchase / User                  | `R`       |
| stock.warehouse.orderpoint        | Purchase / Administrator         | `R`       |
| stock.warehouse.orderpoint        | Inventory / User                 | `R`       |
| stock.warehouse.orderpoint system | Inventory / Administrator        | `R/W/C/D` |

**Record Rules:**

- **stock_warehouse.orderpoint multi-company** (Global) `R/W/C/D`
  - Domain: `[('company_id', 'in', company_ids)]`

### 🗃️ `product.replenish` — Product Replenish
...
| Field                  | Label               | Type        | Req | Store | Relation             |
| ---------------------- | ------------------- | ----------- | --- | ----- | -------------------- |
| `allowed_route_ids`    | Allowed Route       | `many2many` |     |       | stock.route          |
| `allowed_uom_ids`      | Allowed Uom         | `many2many` |     |       | uom.uom              |
| `company_id`           | Company             | `many2one`  |     | ✅    | res.company          |
| `create_date`          | Created on          | `datetime`  |     | ✅    |                      |
| `create_uid`           | Created by          | `many2one`  |     | ✅    | res.users            |
| `date_planned`         | Scheduled Date      | `datetime`  | ✅  | ✅    |                      |
| `display_name`         | Display Name        | `char`      |     |       |                      |
| `forecasted_quantity`  | Forecasted Quantity | `float`     |     |       |                      |
| `forecast_uom_id`      | Unit                | `many2one`  |     |       | uom.uom              |
| `id`                   | ID                  | `integer`   |     | ✅    |                      |
| `product_has_variants` | Has variants        | `boolean`   | ✅  | ✅    |                      |
| `product_id`           | Product             | `many2one`  | ✅  | ✅    | product.product      |
| `product_tmpl_id`      | Product Template    | `many2one`  | ✅  | ✅    | product.template     |
| `product_uom_id`       | Unity of measure    | `many2one`  | ✅  | ✅    | uom.uom              |
| `quantity`             | Quantity            | `float`     | ✅  | ✅    |                      |
| `route_id`             | Preferred Route     | `many2one`  |     | ✅    | stock.route          |
| `show_vendor`          | Show Vendor         | `boolean`   |     |       |                      |
| `supplier_id`          | Vendor              | `many2one`  |     | ✅    | product.supplierinfo |
| `warehouse_id`         | Warehouse           | `many2one`  | ✅  | ✅    | stock.warehouse      |
| `write_date`           | Last Updated on     | `datetime`  |     | ✅    |                      |
| `write_uid`            | Last Updated by     | `many2one`  |     | ✅    | res.users            |
...
| Name                     | Group            | Perms   |
| ------------------------ | ---------------- | ------- |
| access.product.replenish | Inventory / User | `R/W/C` |

### 🗃️ `stock.replenish.mixin` — Product Replenish Mixin
...
### 🗃️ `stock.replenishment.info` — Stock supplier replenishment information
...
| Field                         | Label                         | Type        | Req | Store | Relation                   |
| ----------------------------- | ----------------------------- | ----------- | --- | ----- | -------------------------- |
| `based_on`                    | Based on                      | `selection` | ✅  | ✅    |                            |
| `create_date`                 | Created on                    | `datetime`  |     | ✅    |                            |
| `create_uid`                  | Created by                    | `many2one`  |     | ✅    | res.users                  |
| `display_name`                | Display Name                  | `char`      |     |       |                            |
| `id`                          | ID                            | `integer`   |     | ✅    |                            |
| `json_lead_days`              | Json Lead Days                | `char`      |     |       |                            |
| `json_replenishment_graph`    | Json Replenishment Graph      | `char`      |     |       |                            |
| `orderpoint_id`               | Orderpoint                    | `many2one`  |     | ✅    | stock.warehouse.orderpoint |
| `percent_factor`              | Percent Factor                | `integer`   | ✅  | ✅    |                            |
| `product_id`                  | Product                       | `many2one`  |     |       | product.product            |
| `product_max_qty`             | Max                           | `float`     | ✅  |       |                            |
| `product_min_qty`             | Min                           | `float`     | ✅  |       |                            |
| `product_uom_name`            | Product unit of measure label | `char`      |     |       |                            |
| `qty_to_order`                | To Order                      | `float`     |     |       |                            |
| `show_vendor_tab`             | Show Vendor Tab               | `boolean`   |     |       |                            |
| `supplierinfo_id`             | Vendor Pricelist              | `many2one`  |     |       | product.supplierinfo       |
| `supplierinfo_ids`            | Supplierinfo                  | `many2many` |     | ✅    | product.supplierinfo       |
| `warehouseinfo_ids`           | Resupply Routes               | `one2many`  |     |       | stock.route                |
| `wh_replenishment_option_ids` | Wh Replenishment Option       | `one2many`  |     |       | stock.replenishment.option |
| `write_date`                  | Last Updated on               | `datetime`  |     | ✅    |                            |
| `write_uid`                   | Last Updated by               | `many2one`  |     | ✅    | res.users                  |
...
| Name                     | Group                     | Perms   |
| ------------------------ | ------------------------- | ------- |
| stock.replenishment.info | Inventory / Administrator | `R/W/C` |

### 🗃️ `stock.replenishment.option` — Stock warehouse replenishment option
...
| `create_uid`            | Created by          | `many2one` |     | ✅    | res.users                |
| `display_name`          | Display Name        | `char`     |     |       |                          |
| `free_qty`              | Free Qty            | `float`    |     |       |                          |
| `id`                    | ID                  | `integer`  |     | ✅    |                          |
| `lead_time`             | Lead Time           | `char`     |     |       |                          |
| `location_id`           | Location Stock      | `many2one` |     |       | stock.location           |
| `product_id`            | Product             | `many2one` |     | ✅    | product.product          |
| `qty_to_order`          | To Order            | `float`    |     |       |                          |
| `replenishment_info_id` | Replenishment Info  | `many2one` |     | ✅    | stock.replenishment.info |
| `route_id`              | Route               | `many2one` |     | ✅    | stock.route              |
| `uom`                   | Unit Name           | `char`     |     |       |                          |
| `warehouse_id`          | Supplying Warehouse | `many2one` |     |       | stock.warehouse          |
...
| Name                       | Group            | Perms   |
| -------------------------- | ---------------- | ------- |
| stock.replenishment.option | Inventory / User | `R/W/C` |
...
### 🗃️ `vendor.delay.report` — Vendor Delay Report
...
| -------------- | --------------------- | ---------- | --- | ----- | ---------------- |
| `category_id`  | Product Category      | `many2one` |     | ✅    | product.category |
| `date`         | Effective Date        | `datetime` |     | ✅    |                  |
| `display_name` | Display Name          | `char`     |     |       |                  |
| `id`           | ID                    | `integer`  |     | ✅    |                  |
| `on_time_rate` | On-Time Delivery Rate | `float`    |     | ✅    |                  |
| `partner_id`   | Vendor                | `many2one` |     | ✅    | res.partner      |
| `product_id`   | Product               | `many2one` |     | ✅    | product.product  |
| `qty_on_time`  | On-Time Quantity      | `float`    |     | ✅    |                  |
| `qty_total`    | Total Quantity        | `float`    |     | ✅    |                  |

**Access Rights:**

| Name                     | Group                    | Perms |
| ------------------------ | ------------------------ | ----- |
| vendor.delay.report user | Purchase / User          | `R`   |
| vendor.delay.report      | Purchase / Administrator | `R`   |

### 🗃️ `stock.warehouse` — Warehouse
...
| ------------------------ | ------------------------ | ----------- | --- | ----- | ------------------ |
| `active`                 | Active                   | `boolean`   |     | ✅    |                    |
| `buy_pull_id`            | Buy rule                 | `many2one`  |     | ✅    | stock.rule         |
| `buy_to_resupply`        | Buy to Resupply          | `boolean`   |     |       |                    |
| `code`                   | Short Name               | `char`      | ✅  | ✅    |                    |
| `company_id`             | Company                  | `many2one`  | ✅  | ✅    | res.company        |
| `create_date`            | Created on               | `datetime`  |     | ✅    |                    |
| `create_uid`             | Created by               | `many2one`  |     | ✅    | res.users          |
| `delivery_route_id`      | Delivery Route           | `many2one`  |     | ✅    | stock.route        |
| `delivery_steps`         | Outgoing Shipments       | `selection` | ✅  | ✅    |                    |
| `display_name`           | Display Name             | `char`      |     |       |                    |
| `id`                     | ID                       | `integer`   |     | ✅    |                    |
| `int_type_id`            | Internal Type            | `many2one`  |     | ✅    | stock.picking.type |
| `in_type_id`             | In Type                  | `many2one`  |     | ✅    | stock.picking.type |
| `lot_stock_id`           | Location Stock           | `many2one`  | ✅  | ✅    | stock.location     |
| `mto_pull_id`            | MTO rule                 | `many2one`  |     | ✅    | stock.rule         |
| `name`                   | Warehouse                | `char`      | ✅  | ✅    |                    |
| `out_type_id`            | Out Type                 | `many2one`  |     | ✅    | stock.picking.type |
| `pack_type_id`           | Pack Type                | `many2one`  |     | ✅    | stock.picking.type |
| `partner_id`             | Address                  | `many2one`  |     | ✅    | res.partner        |
| `pick_type_id`           | Pick Type                | `many2one`  |     | ✅    | stock.picking.type |
| `qc_type_id`             | Quality Control Type     | `many2one`  |     | ✅    | stock.picking.type |
| `reception_route_id`     | Receipt Route            | `many2one`  |     | ✅    | stock.route        |
| `reception_steps`        | Incoming Shipments       | `selection` | ✅  | ✅    |                    |
| `resupply_route_ids`     | Resupply Routes          | `one2many`  |     | ✅    | stock.route        |
| `resupply_wh_ids`        | Resupply From            | `many2many` |     | ✅    | stock.warehouse    |
| `route_ids`              | Routes                   | `many2many` |     | ✅    | stock.route        |
| `sequence`               | Sequence                 | `integer`   |     | ✅    |                    |
| `store_type_id`          | Storage Type             | `many2one`  |     | ✅    | stock.picking.type |
| `view_location_id`       | View Location            | `many2one`  | ✅  | ✅    | stock.location     |
| `wh_input_stock_loc_id`  | Input Location           | `many2one`  |     | ✅    | stock.location     |
| `wh_output_stock_loc_id` | Output Location          | `many2one`  |     | ✅    | stock.location     |
| `wh_pack_stock_loc_id`   | Packing Location         | `many2one`  |     | ✅    | stock.location     |
| `wh_qc_stock_loc_id`     | Quality Control Location | `many2one`  |     | ✅    | stock.location     |
| `write_date`             | Last Updated on          | `datetime`  |     | ✅    |                    |
| `write_uid`              | Last Updated by          | `many2one`  |     | ✅    | res.users          |
| `xdock_type_id`          | Cross Dock Type          | `many2one`  |     | ✅    | stock.picking.type |

**Access Rights:**

| Name                    | Group                            | Perms     |
| ----------------------- | -------------------------------- | --------- |
| stock.warehouse.user    | Sales / User: Own Documents Only | `R`       |
| stock.warehouse         | Purchase / User                  | `R`       |
| stock.warehouse         | Purchase / Administrator         | `R`       |
| stock.warehouse.manager | Inventory / Administrator        | `R/W/C/D` |
| stock.warehouse.user    | Role / User                      | `R`       |

**Record Rules:**

- **Warehouse multi-company** (Global) `R/W/C/D`
  - Domain: `[('company_id', 'in', company_ids)]`
