...
> **Module:** `website_event_sale` | **Version:** `19.0.1.0` **Category:** Website |
> **License:** `LGPL-3` **Author:** Odoo S.A. **Website:** _N/A_

## 🔗 Dependencies

[[website_event]] [[event_sale]] [[website_sale]]
...
- `* INHERIT cart_summary_inherit_website_event_sale (qweb)`
...
- `* INHERIT registration_attendee_details (qweb)`
...
### 🗃️ `event.sale.report` — Event Sales Report

Event Registrations-based sales report, allowing to analyze sales and number of seats by
event (type), ticket, etc. Each opened record will also give access to all this
information.
...
| `company_id`                     | Company                                | `many2one`  |     | ✅    | res.company        |
| `display_name`                   | Display Name                           | `char`      |     |       |                    |
| `event_date_begin`               | Event Start Date                       | `date`      |     | ✅    |                    |
| `event_date_end`                 | Event End Date                         | `date`      |     | ✅    |                    |
| `event_id`                       | Event                                  | `many2one`  |     | ✅    | event.event        |
| `event_registration_create_date` | Registration Date                      | `date`      |     | ✅    |                    |
| `event_registration_id`          | Event Registration                     | `many2one`  |     | ✅    | event.registration |
| `event_registration_name`        | Attendee Name                          | `char`      |     | ✅    |                    |
| `event_registration_state`       | Registration Status                    | `selection` |     | ✅    |                    |
| `event_slot_id`                  | Event Slot                             | `many2one`  |     | ✅    | event.slot         |
| `event_ticket_id`                | Event Ticket                           | `many2one`  |     | ✅    | event.event.ticket |
| `event_ticket_price`             | Ticket price                           | `float`     |     | ✅    |                    |
| `event_type_id`                  | Event Type                             | `many2one`  |     | ✅    | event.type         |
| `id`                             | ID                                     | `integer`   |     | ✅    |                    |
| `invoice_partner_id`             | Invoice Address                        | `many2one`  |     | ✅    | res.partner        |
| `is_published`                   | Published Events                       | `boolean`   |     | ✅    |                    |
| `product_id`                     | Product                                | `many2one`  |     | ✅    | product.product    |
| `sale_order_date`                | Order Date                             | `datetime`  |     | ✅    |                    |
| `sale_order_id`                  | Sale Order                             | `many2one`  |     | ✅    | sale.order         |
| `sale_order_line_id`             | Sale Order Line                        | `many2one`  |     | ✅    | sale.order.line    |
| `sale_order_partner_id`          | Customer                               | `many2one`  |     | ✅    | res.partner        |
| `sale_order_state`               | Sale Order Status                      | `selection` |     | ✅    |                    |
| `sale_order_user_id`             | Salesperson                            | `many2one`  |     | ✅    | res.users          |
| `sale_price`                     | Revenues                               | `float`     |     | ✅    |                    |
| `sale_price_untaxed`             | Untaxed Revenues                       | `float`     |     | ✅    |                    |
...
| Name                             | Group                  | Perms |
| -------------------------------- | ---------------------- | ----- |
| access.event.sale.report.manager | Events / Administrator | `R`   |

**Record Rules:**

- **Event Sales Report multi-company** (Global) `R/W/C/D`
