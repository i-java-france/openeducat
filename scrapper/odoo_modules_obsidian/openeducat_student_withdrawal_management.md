---
title: "OpenEduCat Student Withdrawal Management"
module: "openeducat_student_withdrawal_management"
state: "installed"
version: "19.0.1.0"
...
# 🟢 OpenEduCat Student Withdrawal Management

> **Module:** `openeducat_student_withdrawal_management` | **Version:** `19.0.1.0`
> **Category:** Education | **License:** `Other proprietary` **Author:** OpenEduCat Inc
> **Website:** https://www.openeducat.org

## 🔗 Dependencies

[[base]] [[website]] [[openeducat_parent_enterprise]] [[openeducat_core_enterprise]]
[[openeducat_core]]
...
- `Students/General/Student Withdrawal Request`
- `Students/Reporting/Student Withdrawal Analysis`

### Views

- `* INHERIT Add Withdrawal Button to Admission Details Table (qweb)`
- `* INHERIT Portal layout : Student Withdrawal (qweb)`
- `* INHERIT op.student.course.form.inherit (form)`
- `* INHERIT op.student.course.list.inherit (list)`
- `* INHERIT op.student.list.inherit (list)`
- `Thank You (qweb)`
- `Withdrawal Detail (qweb)`
- `Withdrawals Form (qweb)`
- `op.student.withdrawal.form (form)`
- `op.student.withdrawal.list (list)`
- `op.student.withdrawal.pivot (pivot)`
- `student.withdrawal.analysis.wizard.form (form)`
- `student.withdrawal.fee.wizard.form (form)`
...
### 🗃️ `student.withdrawal.analysis.wizard` — Student Withdrawal Analysis Wizard

> ✳️ Transient (Wizard) 📧 Mail Thread
...
| `website_message_ids`        | Website Messages       | `one2many`  |     | ✅    | mail.message          |
| `withdrawal_id`              | Withdrawal             | `many2one`  |     | ✅    | op.student.withdrawal |
| `write_date`                 | Last Updated on        | `datetime`  |     | ✅    |                       |
| `write_uid`                  | Last Updated by        | `many2one`  |     | ✅    | res.users             |
...
| Name                                      | Group       | Perms     |
| ----------------------------------------- | ----------- | --------- |
| access_student_withdrawal_analysis_wizard | Role / User | `R/W/C/D` |

### 🗃️ `op.student.withdrawal` — student withdrawal management
...
| `activity_type_id`              | Next Activity Type            | `many2one`  |     |       | mail.activity.type |
| `activity_user_id`              | Responsible User              | `many2one`  |     |       | res.users          |
| `advisor_id`                    | Advisor                       | `many2one`  |     | ✅    | res.users          |
| `attachment_ids`                | Attachment                    | `many2many` |     | ✅    | ir.attachment      |
| `batch_id`                      | Batch                         | `many2one`  | ✅  | ✅    | op.batch           |
...
| `create_uid`                    | Created by                    | `many2one`  |     | ✅    | res.users          |
| `display_name`                  | Display Name                  | `char`      |     |       |                    |
| `finance_clearance_done`        | Finance Clearance Done?       | `boolean`   |     | ✅    |                    |
| `has_message`                   | Has Message                   | `boolean`   |     |       |                    |
| `id`                            | ID                            | `integer`   |     | ✅    |                    |
| `message_attachment_count`      | Attachment Count              | `integer`   |     |       |                    |
| `message_follower_ids`          | Followers                     | `one2many`  |     | ✅    | mail.followers     |
...
| `principal_id`                  | Principal                     | `many2one`  |     | ✅    | res.users          |
| `rating_ids`                    | Ratings                       | `one2many`  |     | ✅    | rating.rating      |
| `requested_by`                  | Requested By                  | `selection` |     | ✅    |                    |
| `requested_date`                | Requesed Date                 | `datetime`  |     | ✅    |                    |
| `sequence`                      | Index                         | `char`      | ✅  | ✅    |                    |
| `state`                         | Status                        | `selection` |     | ✅    |                    |
| `student_id`                    | Student                       | `many2one`  | ✅  | ✅    | op.student         |
| `website_message_ids`           | Website Messages              | `one2many`  |     | ✅    | mail.message       |
| `withdrawal_reason`             | Withdrawal Reason             | `char`      | ✅  | ✅    |                    |
| `write_date`                    | Last Updated on               | `datetime`  |     | ✅    |                    |
| `write_uid`                     | Last Updated by               | `many2one`  |     | ✅    | res.users          |
...
| Name                         | Group       | Perms     |
| ---------------------------- | ----------- | --------- |
| access_op_student_withdrawal | Role / User | `R/W/C/D` |

### 🗃️ `student.withdrawal.fee.wizard` — Student Withdrawal Fee Action Wizard
...
| `display_name`      | Display Name       | `char`     |     |       |                       |
| `id`                | ID                 | `integer`  |     | ✅    |                       |
| `student_course_id` | Student Course     | `many2one` | ✅  | ✅    | op.student.course     |
| `withdrawal_id`     | Withdrawal Request | `many2one` | ✅  | ✅    | op.student.withdrawal |
| `write_date`        | Last Updated on    | `datetime` |     | ✅    |                       |
| `write_uid`         | Last Updated by    | `many2one` |     | ✅    | res.users             |
...
| Name                                 | Group       | Perms     |
| ------------------------------------ | ----------- | --------- |
| access_student_withdrawal_fee_wizard | Role / User | `R/W/C/D` |
