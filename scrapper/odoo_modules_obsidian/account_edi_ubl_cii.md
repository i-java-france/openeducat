---
title: "Import/Export electronic invoices with UBL/CII"
...
> **Module:** `account_edi_ubl_cii` | **Version:** `19.0.1.0` **Category:** Accounting |
...
XRechnung (UBL). When generating the PDF on the invoice, the PDF will be embedded inside
the xml for all UBL formats. This allows the receiver to retrieve the PDF with only the
xml file. Note that **EHF3 is fully implemented by UBL Bis 3**
([reference](https://anskaffelser.dev/postaward/g3/spec/current/billing-3.0/norway/#_implementation)).
...
German companies, respectively. UBL Bis 3 is only available for companies which country
is present in the [EAS list](https://docs.peppol.eu/poacc/billing/3.0/codelist/eas/).

Note also that in order for Chorus Pro to automatically detect the "PDF/A-3 (Factur-X)"
format, you need to activate the "Factur-X PDF/A-3" option on the journal. This option
will also validate the xml against the Factur-X and Chorus Pro rules and show the
errors.
...
- `* INHERIT account.tax.form.inherit (form)`
- `* INHERIT external_layout_with_context (qweb)`
- `* INHERIT report_invoice_document (qweb)`
...
- `account_invoices_generated_by_odoo (qweb)`

### Reports

- `Invoice report generated by Odoo`
...
### 🗃️ `account.edi.xml.ubl_a_nz` — A-NZ BIS Billing 3.0
...
### 🗃️ `account.edi.ubl` — Base helpers for UBL
...
### 🗃️ `account.edi.xml.ubl_de` — BIS3 DE (XRechnung)
...
### 🗃️ `account.edi.common` — Common functions for EDI documents: generate the data, the constraints, etc
...
### 🗃️ `account.edi.xml.ubl_efff` — E-FFF (BE)
...
### 🗃️ `account.edi.xml.cii` — Factur-x/XRechnung CII 2.2.0
...
### 🗃️ `account.edi.xml.ubl_sg` — SG BIS Billing 3.0
...
### 🗃️ `account.edi.xml.ubl_nl` — SI-UBL 2.0 (NLCIUS)
...
### 🗃️ `account.edi.xml.ubl_20` — UBL 2.0
...
### 🗃️ `account.edi.xml.ubl_21` — UBL 2.1
...
### 🗃️ `account.edi.xml.ubl_bis3` — UBL BIS Billing 3.0.12
