...
class OpStudent(models.Model):
    ...
    _inherit = ['op.student', 'mail.thread', 'mail.activity.mixin', 'rating.mixin', 'website.published.mixin']
    _description = 'Student'

    # ── Fields ──────────────────────────────────────────────
    Job_post_count = fields.Integer(string='Job Post Count', store=False)
    account_move_count = fields.Integer(string='Account Move Count', store=False)
    achievement_line_ids = fields.One2many('op.achievement', 'student_id', string='Achievement Details')
    active = fields.Boolean(string='Active')
    active_lang_count = fields.Integer(string='Active Lang Count', store=False)
    activity_count = fields.Integer(string='Activity Count', store=False)
    ...
    alumni_boolean = fields.Boolean(string='Is An Alumni?')
    ...
    attendance_ids = fields.One2many('op.attendance.line', 'student_id', string='Attendance', help='list of attendances for students')
    ...
    barcode = fields.Char(string='Badge ID', help='ID used for student identification')
    ...
    course_detail_ids = fields.One2many('op.student.course', 'student_id', string='Course Details')
    ...
    crm_lead_count = fields.Integer(string='Crm Lead Count', store=False)
    crm_lead_ids = fields.One2many('crm.lead', 'student_id', string='CRM Leads')
    ...
    discipline_count = fields.Integer(string='Discipline Count', store=False)
    discipline_ids = fields.One2many('op.discipline', 'student_id', string='Discipline Details')
    ...
    fees_detail_ids = fields.One2many('op.student.fees.details', 'student_id', string='Fees Collection Details')
    fees_details_count = fields.Integer(string='Fees Details Count', store=False)
    fees_plan_details_count = fields.Integer(string='Fees Plan Details Count', store=False)
    fees_plan_ids = fields.One2many('op.fees.plan', 'student_id', string='Fees Plan Details')
    ...
    gr_no = fields.Char(string='Registration Number')
    ...
    health_count = fields.Integer(string='Health Count', store=False)
    health_lines = fields.One2many('op.health', 'student_id', string='Health Detail')
    ...
    image_detect = fields.Binary(string='Image Detect')
    ...
    is_finished = fields.Boolean(string='Finished or Withdrawn', help='True if the student has any course             detail with state \'finished\' or \'withdrawn\'.')
    ...
    job_post_ids = fields.One2many('op.job.applicant', 'user_id', string='Job Post Details')
    join_date = fields.Date(string='Join Date', store=False)
    ...
    last_attendance_id = fields.Many2one('op.attendance.line', string='Last Attendance')
    ...
    media_movement_lines = fields.One2many('op.media.movement', 'student_id', string='Movements')
    ...
    number = fields.Char(string='Invoice Number', store=False)
    ...
    pin = fields.Char(string='PIN', help='PIN used to Sign In in Kiosk Mode')
    placement_count = fields.Integer(string='Placement Count', store=False)
    placement_line = fields.One2many('op.placement.offer', 'student_id', string='Placement Details')
    ...
    skill_line = fields.One2many('op.skill.line', 'student_id', string='Skills Details')
    ...
    student_badge_ids = fields.One2many('op.badge.student', 'student_id', string='Badges')
    student_count = fields.Integer(string='Student Count')
    student_skill_line = fields.One2many('op.student.skill.line', 'student_id', string='Student Skills')
    ...
    # Access Rights:
    #   Alumni / Manager: read, write, create, unlink
    #   Alumni / User: read, write, create
    #   OpenEduCat / Manager: read, write, create, unlink
    #   OpenEduCat / User: read, write
    #   Library / Manager: read
    #   Role / Portal: read
    #   Role / Portal: read
    #   Role / Public: read, write, create, unlink

    # Record Rules:
    #   Student Login rule: [('user_id','=',user.id)]
    #   View Students: [(1,'=',1)]
    #   Student Parent Login rule: ['|', ('user_id','=',user.id), ('user_id','in',user.child_ids.ids)]
    #   Student multi-company: ['|',('company_id','=',False),'&',('company_id','in',company_ids),'|',('company_id','child_of',[user.company_id.id]),('company_id','in',user.company_ids.ids)]
    ...
    def _compute_activity_count(self):
        for rec in self:
            rec.activity_count = False  # TODO: implement
