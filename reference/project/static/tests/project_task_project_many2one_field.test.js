...
test("ProjectMany2one: project.task list view", async () => {
    ...
    expect("div[name='project_id'] span.text-danger.fst-italic.text-muted").toHaveCount(
        1
    );
    expect("div[name='project_id'] span.text-danger.fst-italic.text-muted").toHaveText(
        "🔒 Private"
    );
});
