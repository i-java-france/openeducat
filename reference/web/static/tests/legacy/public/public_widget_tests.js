...
QUnit.module("core", {}, function () {
    ...
    QUnit.test("renderElement, no template, default", function (assert) {
        ...
        assert.strictEqual(
            widget.el.nodeName,
            "DIV",
            "should have generated the default element"
        );
        assert.strictEqual(
            widget.el.attributes.length,
            0,
            "should not have generated any attribute"
        );
        assert.ok(
            Object.keys(widget.$el.html() || {}).length === 0,
            "should not have generated any content"
        );
        widget.destroy();
    });
    ...
    QUnit.test("no template, custom tag", function (assert) {
        ...
        assert.strictEqual(
            widget.el.nodeName,
            "UL",
            "should have generated the custom element tag"
        );
        widget.destroy();
    });
    ...
    QUnit.test("no template, @id", function (assert) {
        ...
        assert.hasAttrValue(
            ...
            "id",
            "foo",
            "should have generated the id attribute"
        );
        assert.strictEqual(
            widget.el.id,
            "foo",
            "should also be available via property"
        );
        widget.destroy();
    });
    ...
    QUnit.test("no template, @className", function (assert) {
        ...
        assert.strictEqual(
            widget.el.className,
            "oe_some_class",
            "should have the right property"
        );
        assert.hasAttrValue(
            widget.$el,
            "class",
            "oe_some_class",
            "should have the right attribute"
        );
        widget.destroy();
    });
    ...
    QUnit.test("no template, bunch of attributes", function (assert) {
        ...
        var widget = new (Widget.extend({
            attributes: {
                ...
                clark: "gable",
                // don't read the next line if you care about Harry Potter...
    ...
    QUnit.test("repeated", async function (assert) {
        ...
        await widget.appendTo($fix).then(function () {
            assert.strictEqual(
                $fix.find("p").text(),
                "42",
                "DOM fixture should contain initial value"
            );
            ...
            assert.strictEqual(
                $fix.find("p").text(),
                "36",
                "DOM fixture should use new value"
            );
    ...
    QUnit.test("basic-alias", function (assert) {
        ...
        assert.ok(
            widget.$("li:eq(3)").is(widget.$el.find("li:eq(3)")),
    ...
    QUnit.test("undelegate", async function (assert) {
        ...
        widget.renderElement();
        widget.$el.on("click", "li", function () {
            newclicked = true;
        });
