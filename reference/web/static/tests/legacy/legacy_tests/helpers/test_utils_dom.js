...
const touchEventCancelMapping = (args) =>
    Object.assign({}, touchEventMapping(args), {
        cancelable: false,
    });
...
export async function click(el, options = {}) {
    ...
    const validMatches = options.allowInvisible
        ? matches
        : matches.filter((t) => $(t).is(":visible"));

    if (options.first) {
        if (validMatches.length === 1) {
            throw new Error(
                `There should be more than one visible target ${selectorMsg}.  If` +
        ...
        throw new Error(
            `Found ${validMatches.length} elements to click on, instead of 1 ${selectorMsg}`
        );
    } else {
        target = validMatches[0];
    }
...
export async function triggerEvent(el, eventType, eventAttrs = {}, fast = false) {
    ...
    if (matches.length !== 1) {
        throw new Error(
            `Found ${matches.length} elements to trigger "${eventType}" on, instead of 1 ${selectorMsg}`
        );
    }
