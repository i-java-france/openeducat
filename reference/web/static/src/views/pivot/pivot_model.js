...
export class PivotModel extends Model {
    ...
    _getMeasurements(group, config) {
        ...
        return this._getMeasureSpecs(config).reduce((measurements, measureName) => {
            ...
            if (aggregator === "sum_currency") {
                const currencies =
                    group[
                        metaData.fields[fieldName].currency_field +
                            ":array_agg_distinct"
                    ];
            ...
            this.metaData.activeMeasures.forEach((measureName) => {
                ...
                if (
                    ...
                    JSON.stringify(sortedColumn.groupId) ===
                        JSON.stringify(column.groupId) // FIXME
                ) {
    ...
    _getTableRows(tree, columns) {
        ...
        const subGroupMeasurements = columns.map((column) => {
            ...
            const measurement = {
                ...
                currencyIds,
                isBold:
                    !groupIntersectionId[0].length || !groupIntersectionId[1].length,
            };
            return measurement;
        });
        ...
        rows.push({
            ...
            label:
                indent === 0
                    ? undefined
                    : this.metaData.fields[rowGroupBys[indent - 1].split(":")[0]]
                          .string,
            groupId: rowGroupId,
            indent: indent,
            isLeaf: isLeaf,
    ...
    _sortRows(sortedColumn, config) {
        ...
        const sortFunction = (tree) => (subTreeKey) => {
            ...
            const value =
                this._getCellValue(groupIntersectionId, sortedColumn.measure, {data}) ||
                0;
