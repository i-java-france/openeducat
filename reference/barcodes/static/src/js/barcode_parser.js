export class BarcodeParser {
    ...
    match_pattern(barcode, pattern, encoding) {
        ...
        if (numerical_content) {
            ...
            var decimal_part =
                "0." +
                value_string.substr(
                    decimal_part_match.index,
                    decimal_part_match[0].length - 1
            ...
            match.base_code += barcode.substr(
                num_start + num_length - 2,
                barcode.length - 1
            );
    ...
    parseURI(barcode) {
        ...
        if (identifier === "lgtin" || identifier === "sgtin") {
            ...
            return this.convertURIGTINDataIntoProductAndTrackingNumber(
                barcode,
                data.slice(1)
            );
